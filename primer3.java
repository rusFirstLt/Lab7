...
public class primer3 {
    public static void main(String[] args) {
        try {
            ...
            File f1 = new File("E:\\Lab7\\Primer3.txt");
            if (!f1.exists())
                f1.createNewFile();
            ...
            for (int i = 0; i < kol; i++) {
                ...
                for (int j = 0; j < 20 - fam.length(); j++) rf.writeByte(1); // добавление байтов до 20-ти любой цифрой (=1)

                System.out.print("Введите его должность => ");
                doljnost = sc.next();
                rf.writeUTF(doljnost); // запись должности
                for (int j = 0; j < 20 - doljnost.length(); j++) rf.writeByte(1); // добавление байтов до кол=20 любым числом
